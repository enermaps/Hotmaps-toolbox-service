...
class HeatLoadProfile:
    ...
    def heatloadprofile_nuts_lau(
        year, month, day, nuts, nuts_level
    ):  # /heat-load-profile/nuts-lau
        request_type = ""
            ...
            if q[0]:
                ...
                if request_type == "year":
                    ...
                    data = {
                        ...
                        "granularity": "hour",
                        "unit": "kW",
                        "value": str(round(q[3], constants.NUMBER_DECIMAL_DATA)),
                    }
                output.append(data)
            else:
                output = []
    ...
    def heatloadprofile_hectares(
        year, month, day, geometry
    ):  # /heat-load-profile/hectares
        ...
        # Get the data
        queryData = createQueryDataLPHectares(
            year=year, month=month, day=day, geometry=geometry
        )
    ...
    def duration_curve_nuts_lau(
        year, nuts, nuts_level
    ):  # /heat-load-profile/duration-curve/nuts-lau

        # Get the query
        sql_query = createQueryDataDCNutsLau(
    ...
    def duration_curve_hectares(
        year, geometry
    ):  # /heat-load-profile/duration-curve/hectares
...
def createQueryDataLPHectares(year, month, day, geometry):
    withPart = (
        "with geomInput AS (SELECT ST_Transform(ST_GeomFromText('"
        + geometry
        + "',4326),4258) AS geometry), "
        ...
        + "AND statBySubAreas.gid = stat.load_profile.fk_nuts_gid AND stat.time.year = "
        + str(year)
        + " "
    ...
    selectYear = (
        "min(normalizedCalutation), max(normalizedCalutation), avg(normalizedCalutation), month_of_year "
        + "from normalizedData "
        + "group by month_of_year "
        + "order by month_of_year"
    )
    selectMonth = (
        "min(normalizedCalutation), max(normalizedCalutation), avg(normalizedCalutation), day_of_month "
        + "from normalizedData "
        + "where month_of_year = "
        + str(month)
        + " "
    ...
    selectDay = (
        "normalizedCalutation, hour_of_day "
        + "from normalizedData "
        + "where day_of_month = "
        + str(day)
        + " "
    ...
    queryData = {
        "byYear": {"with": withPart, "select": selectYear},
...
def createQueryDataLPNutsLau(
    year, month, day, nuts, request_type, nuts_level, query_type="heatload"
):
    where_request = ""
    nutsSelectionQuery = ""
    scale_schema = "geo"
    ...
    if nuts_level in constants.scale_level_loadprofile_aggreagtion:
        nutsSelectionQuery = helper.get_nuts_query_selection(
            nuts, scale_level_table, scale_id
        )
    ...
    if query_type == "duration_curve":
        ...
        select_normalized = (
            "avg(stat.load_profile.value / valtot * HDtotal) AS avg_1,min(stat.load_profile.value / valtot * HDtotal) AS min_1, max(stat.load_profile.value / valtot * HDtotal) AS max_1, sum(stat.load_profile.value / valtot * HDtotal) as sum_1,"
            + time_columns
        )
        groupby_normalized = (
            " stat.load_profile.fk_nuts_gid, " "" + group_by_time_columns
        )
        query_select = (
            """select sum(min_1), sum(max_1), sum(avg_1), sum(sum_1), """
    ...
    query_normalized = (
		...
		FROM """
        + from_clause_lp
        + """, heatdemand hd, loadprofile lp, stat.time
		where  """
        + where_clause_lp
        + """ 
...
def createQueryDataDCNutsLau(year, nuts, nuts_level):
    sql_query = createQueryDataLPNutsLau(
        year, None, None, nuts, "year", nuts_level, "duration_curve"
    )
    return sql_query
