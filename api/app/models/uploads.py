...
def generate_tiles(
    upload_folder, grey_tif, layer_type, upload_uuid, user_currently_used_space
):
    """
    ...
    if layer_type != "custom":
        ...
        args_gdal = app.helper.commands_in_array(
    ...
    try:
        ...
        args_tiles = app.helper.commands_in_array(
...
def find_property_column(style_sheet, headers):
    ...
    ns = {"se": "http://www.opengis.net/se", "ogc": "http://www.opengis.net/ogc"}
...
def generate_rule_dictionary(style_sheet):
    ...
    for rule in rules:
        ...
        # identify symbology
        graphic = rule.find(
            "se:PointSymbolizer/se:Graphic/se:Mark/se:WellKnownName", ns
        )
        ...
        if graphic is not None:
            ...
            fill = rule.find(
                "se:PointSymbolizer/se:Graphic/se:Mark/se:Fill/se:SvgParameter", ns
            ).text
            stroke = rule.find(
                "se:PointSymbolizer/se:Graphic/se:Mark/se:Stroke/se:SvgParameter", ns
            ).text
            ...
            # extract graphic
            xpath = "se:PointSymbolizer/se:Graphic".replace(
                "se:", "{http://www.opengis.net/sld}"
            )
            graphic = rule.find(xpath)
            if graphic is not None:
                # extract external graphic
                xpath = "se:ExternalGraphic".replace(
                    "se:", "{http://www.opengis.net/sld}"
                )
                _external_graphic = graphic.find(
                    "se:ExternalGraphic".replace("se:", "{http://www.opengis.net/sld}")
                )
                if _external_graphic is not None:
                    online_resource = _external_graphic.find(
                        "se:OnlineResource".replace(
                            "se:", "{http://www.opengis.net/sld}"
                        ),
                        ns,
                    )
                    xlink_resource = (
                        online_resource.attrib["{0}href".format(ns_xlink)]
                    ...
                    try:
                        graphic_format = _external_graphic.find(
                            "se:Format".replace("se:", "{http://www.opengis.net/sld}")
                        ).text
                    except:
                        graphic_format = ""
                    ...
                    _size = rule.find(
                        "se:PointSymbolizer/se:Graphic/se:Size".replace(
                            "se:", "{http://www.opengis.net/sld}"
                        )
                    )
                    if len(_size.getchildren()) > 0:
                        size_formula = build_sld_size_formula(
                            _size
                        )  # retrieve formula from xml tree
...
def csv_to_geojson(url, layer_type):
    ...
    with open(url, "r", encoding="utf-8-sig") as csvfile:
        ...
        for row in reader:
            ...
            if "external_graphic" in style.keys():
                ...
                try:
                    style["size"] = eval(
                        eg_size_formula.replace(property_column, row[property_column])
                    )
                except:
    ...
    crs = {"type": "name", "properties": {"name": "EPSG:{0}".format(output_srid)}}
