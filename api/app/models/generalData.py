...
def constructWithPartEachLayerHectare(geometry, year, layer, scale_level):
    ...
    if (
        ...
        and scale_level not in layersData[layer]["data_lvl"]
    ):
        return " "
    query = ""
    layer_table_name = (
        layersData[layer]["schema_hectare"] + "." + layersData[layer]["tablename"]
    )
    query_select = "SELECT "
    ...
    if layersData[layer]["table_type"] == vector_type:
        for indic in layersData[layer]["indicators"]:
            if "table_column" in indic:
                query_select += get_indicator_as_query(
                    indic, layer_table_name, layer, scalelvl_column, scale_level
                )
        ...
        if (
            "level_of_data" in layersData[layer]
            and NUTS_LAU_LEVELS[layersData[layer]["level_of_data"]]
            < NUTS_LAU_LEVELS[scale_level]
        ):
            query += (
                " WHERE "
                + year_query
                + " ST_Intersects(st_transform(st_geomfromtext('"
                + geometry
                + "'::text,"
                + CRS_USER_GEOMETRY
                + "),"
                + layersData[layer]["crs"]
                + "),"
                + layer_table_name
                + "."
                + layersData[layer]["geo_column"]
                + ")"
            )
        else:
            query += (
        ...
        query_select = query_select[:-1]
        query_select += (
            " from (select (((ST_SummaryStatsAgg(ST_Clip("
            + layersData[layer]["tablename"]
            + ".rast, 1, st_transform(st_geomfromtext('"
            + geometry
            + "'::text,"
            + CRS_USER_GEOMETRY
            + "),"
        ...
        query += " FROM " + layer_table_name
        query += (
            "  WHERE ST_Intersects(st_transform(st_geomfromtext('"
            + geometry
            + "'::text,"
            + CRS_USER_GEOMETRY
            + "),"
...
def constructWithPartEachLayerNutsLau(nuts, year, layer, scale_level):
    ...
    if scale_level in NUTS_VAlUES:
        ...
        nuts_selection = (
            nust_select_name
            + " as (SELECT geom as geom from geo."
            + name_type
            + " where "
            + name_type
            + ".year = date('"
        ...
        nuts_selection = (
            nust_select_name
            + " as (SELECT st_transform(geom,4326) as geom from public."
            + LAU_TABLE
            + " where "
            + scale_level_name
            + "  in ("
    ...
    layer_table_name = (
        layersData[layer]["schema_scalelvl"] + "." + layersData[layer]["tablename"]
    )
    query_select = "SELECT "

    if (
        layersData[layer]["table_type"] == "raster"
        and layersData[layer]["data_aggregated"]
    ):  # (layersData[layer]['tablename'] != 'wwtp' or layersData[layer]['tablename'] != 'industrial_database' or layersData[layer]['tablename'] != 'wind_50m'):
...
def get_indicator_as_query(
    indic, layer_table_name, layer, scale_level_name, scale_level
):
