...
class AddUploads(Resource):
    ...
    def post(self):
        ...
        args["file"].save(url)
        if file_name.endswith(".tif"):
            generate_tiles.delay(
                upload_folder, url, layer_type, upload_uuid, user_currently_used_space
            )
        else:
            generate_geojson.delay(
                upload_folder, layer_type, upload_uuid, user_currently_used_space
            )
...
class DeleteUploads(Resource):
    ...
    def delete(self):
        ...
        folder_url = (
            USER_UPLOAD_FOLDER + str(user.id) + "/" + str(upload_to_delete.uuid)
        )
...
class ExportRasterNuts(Resource):
    ...
    def post(self=None):
        ...
        # build request
        sql = "WITH buffer " "AS " "( SELECT ST_Buffer(ST_transform(" "(SELECT "
    ...
    def post(self=None):
        ...
        if not str(layers).endswith("_ha"):
            raise RequestException(
                "this is not a correct layer for an hectare selection !"
            )
        # format the layer_name to contain only the name
        layer_name = layers[:-3]
        # build request
        ...
        try:
            for polygon in areas:
                po = shapely_geom.Polygon(
                    [[p["lng"], p["lat"]] for p in polygon["points"]]
                )
                polyArray.append(po)
        except:
