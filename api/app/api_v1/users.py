...
class AskingPasswordRecovery(Resource):
    ...
    def post(self):
        ...
        if User.get_by_email(email) is None:
            return {"message": "request for recovery successful"}
        # mail creation
...
class UserRegistering(Resource):
    ...
    def post(self):
        ...
        try:
            ...
            msg.body = (
                "Welcome " + first_name + " " + last_name + " on the HotMaps toolbox,\n"
...
class LoginUser(Resource):
    ...
    def post(self):
        ...
        # check password
        if not bcrypt.using(salt=constants.FLASK_SALT).verify(
            password, connecting_user.password
        ):
            raise WrongCredentialException
        ...
        return {"message": output, "token": token}
...
class SpaceUsedUploads(Resource):
    ...
    def post(self):
        ...
        # output
        return {"used_size": used_size, "max_size": constants.USER_DISC_SPACE_AVAILABLE}
...
def generate_confirmation_token(email):
    ...
    token = s.dumps({"email": email, "date": str(datetime.datetime.now())})
    user = User.query.filter_by(email=email).first()
...
class FeedbackUser(Resource):
    ...
    def post(self):
        ...
        msg.html = "<h3>{}</h3> \
            ...
            <strong>Description :</strong><p>{}</p>".format(
            title,
            datetime.date.today(),
            name,
            company,
