...
def vector_query_hectares(vector_table_requested, geometry, toCRS):
    ...
    query = (
        "with subAreas as ( SELECT nuts.nuts_id,nuts.gid FROM geo.nuts "
...
def vector_query_nuts(vector_table_requested, area_selected):
    ...
    query = (
        "with selected_zone as ( SELECT geom as geom from geo.nuts where nuts_id IN("
        + area_selected
        + ") "
...
def vector_query_lau(vector_table_requested, area_selected, toCRS):
    ...
    query = (
        "with selected_zone as ( SELECT geom"
        ...
        "AND geo.nuts.year = to_date('2013', 'YYYY') ) select * from stat."
        + vector_table_requested
        + ",subAreas where fk_nuts_gid = subAreas.gid"
    )
...
def nuts_within_the_selection(geometry, toCRS):
    query = (
        "SELECT nuts.nuts_id FROM geo.nuts "
        + "where ST_Intersects( nuts.geom,"
        + " "
        + transformGeo(geometry, toCRS)
        + " ) AND geo.nuts.STAT_LEVL_ = 2 AND year = to_date('2013', 'YYYY')"
    )
    print("query ", query)
