...
def colorize(layer_type, grey_tif, rgb_tif):
    ...
    args_rgba = commands_in_array(
        "gdaldem color-relief {} {} -alpha {} -co COMPRESS=LZW".format(
            grey_tif, grey2rgb_path, rgb_tif
        )
    )
...
def get_style_from_geoserver(layer_type):
    ...
    if b"No such style" in xml:
        ...
        url = (
            constants.GEOSERVER_API_URL
            + "workspaces/hotmaps/styles/"
            + layer_type
            + ".sld"
        )
        result = requests.get(url)
...
def extract_colormap(xml):
    ...
    for color_map in color_map_list:
        ...
        color_map_object = ColorMap(
            color_tuple[0], color_tuple[1], color_tuple[2], opacity, quantity
        )
...
def retrieveCrossIndicator(
    denominator_indicator_name, numerator_indicator_name, layers, payload_output
):
    if denominator_indicator_name in layers and numerator_indicator_name in layers:
        ...
        generateCrossIndicator(
            numerator, denominator, numerator_indicator_name, payload_output
        )
...
def generate_payload_for_compute(inputs_raster_selection, inputs_parameter_selection):
    ...
    data_output.update({"inputs_parameter_selection": inputs_parameter_selection})
    data_output.update({"inputs_raster_selection": inputs_raster_selection})
...
def sampling_data(listValues):
    ...
    for n, l in enumerate(listValues):
        listPoints.append({"X": n + 1, "Y": listValues[n]})
...
def get_nuts_query_selection(nuts, scale_level_table, scale_id):
    ...
    if scale_level_table == "nuts":
        ...
        return (
            """nutsSelection as (
            SELECT nuts.nuts_id as nuts2_id, tbl2."""
            + scale_id
            + """ as scale_id
            from geo.nuts nuts, """
            + scale_schema
            ...
            + scale_level_table
            + """ tbl2
            where tbl2.year = date('2013-01-01') and tbl2."""
            + scale_id
            + """ in ("""
            + nuts
            + """)
            and st_within(st_transform(tbl2.geom,"""
            + constants.CRS_NUTS
            + """),nuts.geom)
            and nuts.stat_levl_ = 2
            group by nuts.nuts_id, tbl2."""
            + scale_id
            + """),"""
        )
