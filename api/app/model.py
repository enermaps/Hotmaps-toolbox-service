...
def addRegisterCalulationModule(data):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO calculation_module (cm_id, cm_name, cm_description, category, cm_url, layers_needed, createdAt, updateAt,type_layer_needed,authorized_scale,description_link,vectors_needed,wiki_url) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)",
        (
            cm_Id,
            cm_name,
            cm_description,
...
def init_sqlite_caculation_module_database(dbname=DB_NAME):
    ...
    cursor.execute(
        "CREATE TABLE calculation_module (cm_id INTEGER NOT NULL, cm_name VARCHAR(255), wiki_url VARCHAR(255),"
    ...
    cursor.execute(
        "CREATE TABLE inputs_calculation_module (input_id INTEGER NOT NULL, input_name VARCHAR(255), "
...
def register_calulation_module(data):
    if data is not None:
        ...
        try:
            ...
            cursor.execute(
                "INSERT INTO calculation_module (cm_id, cm_name, cm_description, category, cm_url, layers_needed, createdAt, updatedAt, type_layer_needed,authorized_scale,description_link,vectors_needed,type_vectors_needed,wiki_url) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                (
                    cm_id,
                    cm_name,
                    cm_description,
            ...
            for value in inputs_calculation_module:
                ...
                cursor = conn.cursor()
                cursor.execute(
                    "INSERT INTO inputs_calculation_module (input_name, input_type, input_parameter_name, input_value,input_priority, input_unit, input_min, input_max, cm_id, createdAt,updatedAt) VALUES (?,?,?,?,?,?,?,?,?,?,?)",
                    (
                        input_name,
                        input_type,
...
def update_calulation_module(
    ...
    try:
        ...
        cursor.execute(
            "UPDATE calculation_module SET cm_name = ?, cm_description = ?, category= ?,  cm_url= ?,  layers_needed= ?,  createdAt= ?,  updatedAt = ? ,  type_layer_needed = ?, authorized_scale = ?,description_link = ?, vectors_needed = ?, type_vectors_needed=?, wiki_url=? WHERE cm_id = ? ",
            (
                cm_name,
                cm_description,
                category,
        ...
        conn.commit()
        cursor.execute(
            "DELETE FROM inputs_calculation_module WHERE cm_id = ? ", (str(cm_id))
        )
        conn.commit()
        for value in inputs_calculation_module:
            input_name = value["input_name"]
...
def getUI(cm_id):
    ...
    results = cursor.execute(
        "select * from inputs_calculation_module where cm_id = ?", (cm_id,)
    )
    conn.commit()
...
def delete_cm_ui_with_id(cm_id):
    try:
        ...
        results = cursor.execute(
            "DELETE FROM inputs_calculation_module WHERE cm_id = ?", (cm_id)
        )
        conn.commit()
        conn.close()
        return results
...
def delete_cm_with_id(cm_id):
    try:
        ...
        results = cursor.execute(
            "DELETE FROM calculation_module WHERE cm_id = ?", (cm_id)
        )
        conn.commit()
        conn.close()
        return results
...
def getCMList():
    response = helper.retrieve_list_from_sql_result(
        query_calculation_module_database("select * from calculation_module ")
    )
    return response
...
def get_shapefile_from_selection(
    scalevalue, id_selected_list, ouput_directory, EPSG=str(3035)
):
    ...
    if scalevalue == "nuts":
        subprocess.call(
            ...
            + '" -sql "select ST_Union(ST_Transform(geom,'
            + EPSG
            + ")) from geo.nuts where nuts_id IN ("
            + id_selected_list
            + ') AND year = date({})"'.format("'2013-01-01'"),
            shell=True,
        )
        ...
        subprocess.call(
            ...
            + '" -sql "select ST_Union(ST_Transform(geom,'
            + EPSG
            + ")) from public.tbl_lau1_2 where comm_id IN ("
            + id_selected_list
            + ')"',
            shell=True,
        )
...
def get_raster_from_csv(wkt_point, layer_needed, output_directory):
    ...
    filename_csv = helper.write_wkt_csv(
        helper.generate_csv_name(output_directory), wkt_point_3035
    )
...
def clip_raster_from_shapefile(shapefile_path, layer_needed, output_directory):
    ...
    for layer in layer_needed:
        ...
        args = commands_in_array(
            "gdalwarp -dstnodata 0 -cutline {} -crop_to_cutline -of GTiff {} {} -co COMPRESS=DEFLATE".format(
                shapefile_path, path_to_dataset, filename_tif
            )
        )
        run_command(args)
        inputs_raster_selection[type] = filename_tif
...
def retrieve_vector_data_for_calculation_module(
    vectors_needed, scalevalue, area_selected
):
    """
    ...
    for vector_table_requested in vectors_needed:
        ...
        if layer_id == 0:
            if scalevalue == "hectare":
                layer_path = ExportCut.cut_hectares(
                    area_selected, layer_type + "_ha", "public", "2012"
                )
                print(layer_type + "_ha")
            else:
                layer_path = ExportCut.cut_nuts(
                    layer_type + "_" + scalevalue, area_selected, "public", "2012"
                )
        else:

            upload = Uploads.query.filter_by(id=layer_id).first()
            path_to_dataset = upload.url
            layer_path = ExportCut.cut_personal_layer(
                scalevalue, path_to_dataset, area_selected
            )["path"]
...
def get_vectors_needed(cm_id):
    conn = myCMpool.connect()
    cursor = conn.cursor()
    vectors_needed = cursor.execute(
        "select vectors_needed from calculation_module where cm_id = ?", (cm_id)
    )
    conn.commit()
...
def get_cutline_input(areas, scalelevel, data_type):
    if scalelevel == "hectare":
        ...
        return get_shapefile_from_selection(
            scalelevel, areas, constants.UPLOAD_DIRECTORY, "4326"
        )
    # if data_type == 'raster':
    #     if scalelevel == 'hectare':
    #         areas = area_to_geom(areas)
...
class ExportCut:
    ...
    def cut_nuts(layers: str, nuts: list, schema: str, year: str):
        ...
        :return:
        """
        csv_result = get_csv_from_nuts(
            layers=layers, nuts=nuts, schema=schema, year=year
        )
    ...
    def cut_personal_layer(scale_level, upload_url, areas):
        ...
        if scale_level == "hectare":
            ...
            cutline_input = get_shapefile_from_selection(
                scale_level[:-1], areas, UPLOAD_DIRECTORY, "4326"
            )
        print(cutline_input)
    ...
    def cut_hectares(areas: list, layers: str, schema: str, year: str):
        ...
        :return:
        """
        csv_result = get_csv_from_hectare(
            areas=areas, layers=layers, schema=schema, year=year
        )
        return ExportCut.save_file_csv_random_name(content=csv_result)
...
def get_csv_from_nuts(layers, nuts, schema, year):
    ...
    date_sql = """SELECT timestamp FROM {0}.{1} GROUP BY timestamp ORDER BY timestamp DESC;""".format(
        schema, layer_name
    )
    try:
...
def get_csv_from_hectare(areas, layers, schema, year):
    ...
    sql = """SELECT ST_ASTEXT({3}) as geometry_wkt, ST_SRID({3}) as srid, * 
        ...
        schema, layer_name, layer_year, geom_col_name, str(multipolygon)
    )
    # execute query
    try:
        result = db.engine.execute(sql)
